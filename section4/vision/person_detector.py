#!/usr/bin/env python3
"""Smart Guard Section 4 vision relay.
...
CONTROL_FILE = Path(os.getenv("SMART_GUARD_VISION_CONTROL_FILE", "/run/smart-guard/vision_control.json"))
...
MODEL_DIR = Path(os.getenv("SMART_GUARD_VISION_MODEL_DIR", "/opt/smart-guard/section4/vision/models"))
...
def draw_overlay(
    ...
    lines = [
        ...
        f"Guard: {'ARMED' if guard_armed else 'DISARMED'}",
        f"Thermal profile: {profile.mode.upper()} / detect {profile.detection_width}px",
    ]
...
def process_client(client: socket.socket, peer: tuple[str, int], detector: PersonDetector) -> None:
    ...
    try:
        ...
        log("raw camera disconnected; heartbeat intentionally stops for watchdog detection")
