...
static char g_guard_state_file[512] = "/var/lib/smart-guard/guard_mode";
static char g_blackbox_database_file[512] = "/var/lib/smart-guard/blackbox.db";
static char g_thermal_status_file[512] = "/run/smart-guard/thermal_status.json";
static char g_vision_status_file[512] = "/run/smart-guard/vision_status.json";
static int g_blackbox_capacity = DEFAULT_BLACKBOX_CAPACITY;
...
"<style>"
":root{color-scheme:dark;--bg:#0f172a;--card:#182338;--line:#31405a;--text:#edf3ff;--muted:#9fb0ca;--ok:#43d17d;--warn:#ffb84d;--danger:#ff5c70;}"
...
".label{color:var(--muted);font-size:.9rem}.value{font-size:clamp(1.35rem,3.5vw,2.1rem);font-weight:700;margin-top:8px}.camera{margin-top:14px}.camera img{display:block;width:100%;aspect-ratio:16/9;object-fit:contain;background:#05080e;border-radius:14px;border:1px solid var(--line)}"
".guard{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-top:14px}.guard button{border:0;border-radius:12px;padding:12px 18px;font-weight:800;cursor:pointer;background:var(--danger);color:white}.guard button.disarmed{background:#3a4964}.guard button:disabled{opacity:.55;cursor:wait}"
".footer{margin-top:16px;color:var(--muted);font-size:.9rem}.ok{color:var(--ok)}.warn{color:var(--warn)}.danger{color:var(--danger)}.links{margin-top:14px}.links a{color:#8ec5ff;margin-right:18px}"
"@media(max-width:850px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:480px){.grid{grid-template-columns:1fr}.wrap{padding:15px}.guard{align-items:flex-start;flex-direction:column}}"
"</style>"
"</head>"
"<body>"
"<main class=\"wrap\">"
"<section class=\"top\"><div><h1>Smart Guard System</h1><div class=\"sub\">Student: " STUDENT_NAME " &nbsp;|&nbsp; ID: " STUDENT_ID "</div></div><div class=\"pill\">Section 4: Guard + Black Box + Watchdog + Thermal</div></section>"
"<section class=\"grid\">"
...
"<article class=\"card\"><div class=\"label\">Black-box total events</div><div class=\"value\" id=\"totalEvents\">0</div></article>"
"<article class=\"card\"><div class=\"label\">Stored ring records</div><div class=\"value\" id=\"storedEvents\">0</div></article>"
"<article class=\"card\"><div class=\"label\">Adaptive thermal mode</div><div class=\"value\" id=\"thermalMode\">--</div></article>"
"<article class=\"card\"><div class=\"label\">Last frame age</div><div class=\"value\" id=\"frameAge\">--</div></article>"
"</section>"
"<section class=\"card guard\"><div><div class=\"label\">Anti-theft guard mode</div><div class=\"value\" id=\"guardText\">Loading...</div><div class=\"sub\">When armed, each new positive detection sends an immediate email with snapshot and MQTT alarm.</div></div><button id=\"guardButton\" onclick=\"toggleGuard()\">Toggle</button></section>"
"<section class=\"card camera\"><div class=\"label\">Live annotated MJPEG stream</div><img src=\"/api/v1/stream\" alt=\"Live camera stream\"></section>"
"<div class=\"links\"><a href=\"https://HOST_PLACEHOLDER:8443/docs\">Swagger UI (port 8443)</a><a href=\"/api/v1/blackbox?limit=20\">Black-box JSON</a><a href=\"/api/v1/thermal\">Thermal status</a></div>"
"<div class=\"footer\"><span id=\"status\" class=\"warn\">Waiting for telemetry...</span> &nbsp; Last update: <span id=\"timestamp\">--</span></div>"
"</main>"
"<script>"
"const $=id=>document.getElementById(id);const mb=kb=>(kb/1024).toFixed(1)+' MB';let guardArmed=false;"
"function renderGuard(){const b=$('guardButton');$('guardText').textContent=guardArmed?'ARMED':'DISARMED';$('guardText').className='value '+(guardArmed?'danger':'ok');b.textContent=guardArmed?'Disarm guard':'Arm guard';b.className=guardArmed?'':'disarmed';}"
"async function toggleGuard(){const b=$('guardButton');b.disabled=true;let token=sessionStorage.getItem('smartGuardCommandToken');if(!token){token=prompt('Enter X-Command-Token from /etc/smart-guard/section2.env');if(token)sessionStorage.setItem('smartGuardCommandToken',token);}if(!token){b.disabled=false;return;}try{const r=await fetch('/api/v1/guard',{method:'POST',headers:{'Content-Type':'application/json','X-Command-Token':token},body:JSON.stringify({enabled:!guardArmed})});if(r.status===401){sessionStorage.removeItem('smartGuardCommandToken');throw new Error('Invalid command token');}if(!r.ok)throw new Error(await r.text());const data=await r.json();guardArmed=data.enabled;renderGuard();}catch(e){alert('Guard update failed: '+e.message);}finally{b.disabled=false;}}"
"async function update(){try{const [tr,pr,gr,br,hr]=await Promise.all([fetch('/api/v1/telemetry',{cache:'no-store'}),fetch('/api/v1/persons',{cache:'no-store'}),fetch('/api/v1/guard',{cache:'no-store'}),fetch('/api/v1/blackbox?limit=1',{cache:'no-store'}),fetch('/api/v1/thermal',{cache:'no-store'})]);if(!tr.ok||!pr.ok||!gr.ok||!br.ok||!hr.ok)throw new Error('HTTP error');const t=await tr.json(),p=await pr.json(),g=await gr.json(),bb=await br.json(),th=await hr.json();$('cpu').textContent=t.cpu_usage_percent.toFixed(1)+' %';$('mem').textContent=mb(t.memory_available_kb);$('persons').textContent=p.persons;$('temp').textContent=t.cpu_temperature_available?t.cpu_temperature_c.toFixed(1)+' °C':'Unavailable';$('frameAge').textContent=t.last_frame_age_seconds>=0?t.last_frame_age_seconds.toFixed(1)+' s':'--';$('totalEvents').textContent=bb.total_detection_events;$('storedEvents').textContent=bb.stored_records+' / '+bb.capacity;$('thermalMode').textContent=(th.mode||'normal').toUpperCase();$('thermalMode').className='value '+(th.thermal_active?'warn':'ok');guardArmed=!!g.enabled;renderGuard();$('timestamp').textContent=t.timestamp;$('status').textContent=t.camera_connected?'System online':'Waiting for camera / watchdog active';$('status').className=t.camera_connected?'ok':'warn';}catch(e){$('status').textContent='Update error: '+e.message;$('status').className='warn';}}update();setInterval(update,2000);"
"</script>"
"</body></html>";
...
static int get_env_int_range(const char *name, int fallback, int minimum, int maximum)
{
    const char *value = getenv(name);
    char *end = NULL;
    long parsed;
...
static bool peer_is_allowed(const struct sockaddr_in *peer)
{
    ...
    /* Section 3 camera path:
     *   physical host -> vision relay (TCP 9200) -> C web core (TCP 9100)
     * Temperature packets still arrive directly from the configured physical
     * host, while annotated JPEG frames are forwarded from loopback.  No other
     * source address is accepted.
     */
...
static int atomic_write_text_file(const char *path, const char *text, mode_t mode)
{
    char temporary[1024];
    FILE *file;
...
static bool read_guard_state(void)
{
    FILE *file = fopen(g_guard_state_file, "r");
    char text[32] = "0";
    if (file == NULL) return false;
    if (fgets(text, sizeof(text), file) == NULL) text[0] = '0';
    fclose(file);
    return text[0] == '1' || strncasecmp(text, "true", 4U) == 0 || strncasecmp(text, "armed", 5U) == 0;
}

static enum MHD_Result authorize_control_request(struct MHD_Connection *connection)
{
...
static enum MHD_Result serve_guard(struct MHD_Connection *connection, const char *method, const char *body)
{
    char timestamp[64];
    char response[512];
    bool enabled = read_guard_state();
    format_timestamp(timestamp, sizeof(timestamp));
    if (strcmp(method, MHD_HTTP_METHOD_GET) == 0) {
        snprintf(response, sizeof(response),
                 "{\"student_id\":\"%s\",\"enabled\":%s,\"state\":\"%s\",\"timestamp\":\"%s\"}",
                 STUDENT_ID, enabled ? "true" : "false", enabled ? "armed" : "disarmed", timestamp);
        return queue_text(connection, MHD_HTTP_OK, response, "application/json; charset=utf-8");
    }
    if (strcmp(method, MHD_HTTP_METHOD_POST) != 0) {
        return queue_text(connection, MHD_HTTP_METHOD_NOT_ALLOWED,
                          "{\"error\":\"method not allowed\"}", "application/json; charset=utf-8");
    }
    enum MHD_Result auth = authorize_control_request(connection);
    if (auth != MHD_YES) return auth;
    cJSON *root = cJSON_Parse(body != NULL ? body : "");
    cJSON *enabled_item = root != NULL ? cJSON_GetObjectItemCaseSensitive(root, "enabled") : NULL;
    if (!cJSON_IsBool(enabled_item)) {
        cJSON_Delete(root);
        return queue_text(connection, MHD_HTTP_BAD_REQUEST,
                          "{\"error\":\"JSON body must contain boolean field enabled\"}",
                          "application/json; charset=utf-8");
    }
    enabled = cJSON_IsTrue(enabled_item);
    cJSON_Delete(root);
    if (atomic_write_text_file(g_guard_state_file, enabled ? "1\n" : "0\n", 0640) != 0) {
        return queue_text(connection, MHD_HTTP_INTERNAL_SERVER_ERROR,
                          "{\"error\":\"cannot persist guard state\"}",
                          "application/json; charset=utf-8");
    }
    format_timestamp(timestamp, sizeof(timestamp));
    snprintf(response, sizeof(response),
             "{\"student_id\":\"%s\",\"enabled\":%s,\"state\":\"%s\",\"changed_at\":\"%s\"}",
             STUDENT_ID, enabled ? "true" : "false", enabled ? "armed" : "disarmed", timestamp);
    fprintf(stderr, "Guard mode changed to %s via API\n", enabled ? "ARMED" : "DISARMED");
    return queue_text(connection, MHD_HTTP_OK, response, "application/json; charset=utf-8");
}

static long long sqlite_metadata_value(sqlite3 *db, const char *key)
{
    sqlite3_stmt *statement = NULL;
    long long value = 0;
    if (sqlite3_prepare_v2(db, "SELECT value FROM metadata WHERE key=?1;", -1, &statement, NULL) != SQLITE_OK) {
        return 0;
    }
...
static enum MHD_Result serve_blackbox(struct MHD_Connection *connection)
{
    int limit = 20;
    const char *limit_text = MHD_lookup_connection_value(connection, MHD_GET_ARGUMENT_KIND, "limit");
    if (limit_text != NULL && limit_text[0] != '\0') {
        char *end = NULL;
        long parsed = strtol(limit_text, &end, 10);
        if (end != limit_text && *end == '\0' && parsed >= 0 && parsed <= MAX_BLACKBOX_LIMIT) limit = (int)parsed;
    }
    sqlite3 *db = NULL;
    cJSON *root = cJSON_CreateObject();
    cJSON *records = cJSON_CreateArray();
    if (root == NULL || records == NULL) {
        cJSON_Delete(root); cJSON_Delete(records); return MHD_NO;
    }
    cJSON_AddStringToObject(root, "student_id", STUDENT_ID);
    cJSON_AddNumberToObject(root, "capacity", g_blackbox_capacity);
    cJSON_AddBoolToObject(root, "guard_enabled", read_guard_state());
    if (sqlite3_open_v2(g_blackbox_database_file, &db, SQLITE_OPEN_READONLY | SQLITE_OPEN_FULLMUTEX, NULL) != SQLITE_OK) {
        if (db != NULL) sqlite3_close(db);
        cJSON_AddNumberToObject(root, "stored_records", 0);
        cJSON_AddNumberToObject(root, "total_detection_events", 0);
        cJSON_AddNumberToObject(root, "total_persons_detected", 0);
        cJSON_AddItemToObject(root, "records", records);
    } else {
        sqlite3_busy_timeout(db, 2000);
        sqlite3_stmt *count_statement = NULL;
        long long stored = 0;
        if (sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM detections;", -1, &count_statement, NULL) == SQLITE_OK &&
            sqlite3_step(count_statement) == SQLITE_ROW) stored = sqlite3_column_int64(count_statement, 0);
        sqlite3_finalize(count_statement);
        cJSON_AddNumberToObject(root, "stored_records", (double)stored);
        cJSON_AddNumberToObject(root, "total_detection_events",
                                (double)sqlite_metadata_value(db, "total_detection_events"));
        cJSON_AddNumberToObject(root, "total_persons_detected",
                                (double)sqlite_metadata_value(db, "total_persons_detected"));
        sqlite3_stmt *statement = NULL;
        if (sqlite3_prepare_v2(db,
                "SELECT id,event_id,detected_at,persons,temperature_c,vision_fps,snapshot_path,guard_armed "
                "FROM detections ORDER BY id DESC LIMIT ?1;", -1, &statement, NULL) == SQLITE_OK) {
            sqlite3_bind_int(statement, 1, limit);
            while (sqlite3_step(statement) == SQLITE_ROW) {
                cJSON *record = cJSON_CreateObject();
                const unsigned char *event_id = sqlite3_column_text(statement, 1);
                const unsigned char *detected_at = sqlite3_column_text(statement, 2);
                const unsigned char *snapshot_path = sqlite3_column_text(statement, 6);
                cJSON_AddNumberToObject(record, "id", (double)sqlite3_column_int64(statement, 0));
                cJSON_AddStringToObject(record, "event_id", event_id != NULL ? (const char *)event_id : "");
                cJSON_AddStringToObject(record, "timestamp", detected_at != NULL ? (const char *)detected_at : "");
                cJSON_AddNumberToObject(record, "persons", sqlite3_column_int(statement, 3));
                if (sqlite3_column_type(statement, 4) == SQLITE_NULL) cJSON_AddNullToObject(record, "temperature_c");
                else cJSON_AddNumberToObject(record, "temperature_c", sqlite3_column_double(statement, 4));
                cJSON_AddNumberToObject(record, "vision_fps", sqlite3_column_double(statement, 5));
                cJSON_AddStringToObject(record, "snapshot_path", snapshot_path != NULL ? (const char *)snapshot_path : "");
                cJSON_AddBoolToObject(record, "guard_armed", sqlite3_column_int(statement, 7) != 0);
    ...
    cJSON_Delete(root);
    if (text == NULL) return MHD_NO;
    enum MHD_Result result = queue_text(connection, MHD_HTTP_OK, text, "application/json; charset=utf-8");
    free(text);
    return result;
}

static enum MHD_Result serve_json_status_file(struct MHD_Connection *connection,
                                               const char *path,
                                               const char *fallback)
{
    size_t length = 0U;
    char *text = read_entire_file(path, &length);
    if (text == NULL || length == 0U) {
        free(text);
        return queue_text(connection, MHD_HTTP_OK, fallback, "application/json; charset=utf-8");
    }
    enum MHD_Result result = queue_text(connection, MHD_HTTP_OK, text, "application/json; charset=utf-8");
    free(text);
    return result;
...
)
{
    ...
    if (path_is(url, "/api/v1/guard")) {
        return serve_guard(connection, method, body != NULL ? body : "");
    }
    if (path_is(url, "/api/v1/blackbox")) {
        if (strcmp(method, MHD_HTTP_METHOD_GET) != 0) goto method_not_allowed;
        return serve_blackbox(connection);
    }
    if (path_is(url, "/api/v1/thermal")) {
        if (strcmp(method, MHD_HTTP_METHOD_GET) != 0) goto method_not_allowed;
        return serve_json_status_file(connection, g_thermal_status_file,
            "{\"thermal_active\":false,\"mode\":\"normal\",\"status\":\"not initialized\"}");
    }
    if (path_is(url, "/api/v1/vision-status")) {
        if (strcmp(method, MHD_HTTP_METHOD_GET) != 0) goto method_not_allowed;
        return serve_json_status_file(connection, g_vision_status_file,
            "{\"status\":\"no frame heartbeat yet\"}");
    }
...
int main(void)
{
    ...
    snprintf(g_guard_state_file, sizeof(g_guard_state_file), "%s",
             get_env_string("SMART_GUARD_GUARD_STATE_FILE", "/var/lib/smart-guard/guard_mode"));
    snprintf(g_blackbox_database_file, sizeof(g_blackbox_database_file), "%s",
             get_env_string("SMART_GUARD_BLACKBOX_DB", "/var/lib/smart-guard/blackbox.db"));
    snprintf(g_thermal_status_file, sizeof(g_thermal_status_file), "%s",
             get_env_string("SMART_GUARD_THERMAL_STATUS_FILE", "/run/smart-guard/thermal_status.json"));
    snprintf(g_vision_status_file, sizeof(g_vision_status_file), "%s",
             get_env_string("SMART_GUARD_VISION_STATUS_FILE", "/run/smart-guard/vision_status.json"));
    g_blackbox_capacity = get_env_int_range("SMART_GUARD_BLACKBOX_CAPACITY", DEFAULT_BLACKBOX_CAPACITY, 5, 100000);
    ...
    if (!g_stop) {
        fprintf(stderr,
                "Smart Guard Section 4 started: http=%d https=%d internal_api=127.0.0.1:%d host=%s student=%s\n",
    ...
    free(certificate);
    free(private_key);
    fprintf(stderr, "Smart Guard Section 4 stopped cleanly\n");
    return EXIT_SUCCESS;
}
