"""Thin FastAPI/Swagger gateway for Smart Guard Section 4.
...
command_token_scheme = APIKeyHeader(
    ...
    description=(
        ...
        "It is required for POST /api/v1/command and POST /api/v1/guard."
    ),
    auto_error=False,
)
...
app = FastAPI(
    ...
    version="4.0.0",
    description=(
        "Section 4 REST API for Amir Hossein Motiei (401102553). "
...
class GuardRequest(BaseModel):
    enabled: bool = Field(description="Arm or disarm anti-theft guard mode.", examples=[True])


class GuardResponse(BaseModel):
    student_id: str = Field(examples=[STUDENT_ID])
    enabled: bool = Field(examples=[True])
    state: str = Field(examples=["armed"])
    timestamp: str | None = Field(default=None)
    changed_at: str | None = Field(default=None)


class BlackBoxRecord(BaseModel):
    id: int = Field(ge=1)
    event_id: str
    timestamp: str
    persons: int = Field(ge=1)
    temperature_c: float | None = None
    vision_fps: float = Field(ge=0)
    snapshot_path: str
    guard_armed: bool
...
class BlackBoxResponse(BaseModel):
    student_id: str = Field(examples=[STUDENT_ID])
    capacity: int = Field(ge=5)
    guard_enabled: bool
    stored_records: int = Field(ge=0)
    total_detection_events: int = Field(ge=0)
    total_persons_detected: int = Field(ge=0)
...
@app.get(
    ...
    tags=["Section 4"],
    summary="Read anti-theft guard mode",
)
async def guard_status() -> Any:
    return await _json_request("GET", "/api/v1/guard")
...
@app.post(
    ...
    response_model_exclude_none=True,
    tags=["Section 4"],
    summary="Arm or disarm anti-theft guard mode",
...
async def set_guard(
    ...
    return await _json_request(
        "POST", "/api/v1/guard", json_body=request.model_dump(), headers=headers
    )
...
@app.get(
    ...
    tags=["Section 4"],
    summary="SQLite circular black-box report",
)
async def blackbox(
    limit: int = Query(default=20, ge=0, le=100, description="Newest records to return."),
) -> Any:
    return await _json_request("GET", f"/api/v1/blackbox?limit={limit}")
...
@app.get(
    ...
    response_model_exclude_none=True,
    tags=["Section 4"],
    summary="Adaptive thermal-management state",
)
async def thermal() -> Any:
    return await _json_request("GET", "/api/v1/thermal")


@app.get(
    "/api/v1/vision-status",
    tags=["Section 4"],
    summary="Latest vision heartbeat and active processing profile",
)
async def vision_status() -> Any:
    return await _json_request("GET", "/api/v1/vision-status")
...
@app.get("/API/V1/GUARD", include_in_schema=False)
async def guard_upper() -> Any:
    return await guard_status()


@app.post("/API/V1/GUARD", include_in_schema=False)
async def set_guard_upper(
    request: GuardRequest,
    ...
    return await _json_request("POST", "/api/v1/guard", json_body=request.model_dump(), headers=headers)


@app.get("/API/V1/BLACKBOX", include_in_schema=False)
async def blackbox_upper(limit: int = Query(default=20, ge=0, le=100)) -> Any:
    return await blackbox(limit=limit)
